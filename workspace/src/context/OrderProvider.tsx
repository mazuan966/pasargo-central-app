        ...
        if (originalOrderId) {
            ...
            await runTransaction(db, async (transaction) => {
                ...
                const updatedTotal = originalOrderData.total + total;
                 ...
                 if (originalOrderData.paymentMethod === 'FPX (Toyyibpay)') {
                    const billResponse = await createToyyibpayBill(originalOrderData.orderNumber, total, userData);
            ...
            await runTransaction(db, async (transaction) => {
                ...
                if (paymentMethod === 'toyyibpay') {
                    const billResponse = await createToyyibpayBill(newOrderNumber, total, userData);
...
async function createToyyibpayBill(orderNumber: string, total: number, user: User) {
    ...
    const billParams = new URLSearchParams({
        ...
        'billReturnUrl': `${appUrl}/payment/status`,
